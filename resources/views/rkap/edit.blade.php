    ...
    <div class="row">
        <div class="col-md-9 col-sm-9 col-xs-9">
            <div class="x_panel">
                ...
                <div class="x_content">
                    <br />
                    <form method="POST" action="{{ route('rkap.update', $template->id) }}" data-parsley-validate class="form-horizontal form-label-left" enctype="multipart/form-data">
                        {!! csrf_field() !!}
                        {!! method_field('PUT') !!}
                        <div class="form-group">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select name="fase_id" class="form-control">
                                    @foreach($fase as $row)
                                        <option value="{{ $row->id }}" {{ (old('fase_id', $template->fase_id) == $row->id)?'selected':'' }}>{{ $row->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group{{ $errors->has('tahun') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <input type="text" name="tahun" value="{{ old('tahun', $template->tahun) }}" required="required" class="form-control col-md-7 col-xs-12">
                                @if ($errors->has('tahun'))

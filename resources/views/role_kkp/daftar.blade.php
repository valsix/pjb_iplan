                    ...
                    <div class="col-md-12">
                        <table id="fasyankes_list" class="table table-striped table-responsive table-bordered dataTable" >
                            ...
                            <tbody class="filterable">
                                <?php $i = 1;?>
                                @foreach($rolekkp as $item)
                                <tr class="filterable">
                                    <td class="text-center">{{$i++}}</td>
                                    <td class="text-left" >{{$item->display_name}}</td>
									<td class="text-center" >{{dateIdnFromTimestamp($item->created_at)}}</td>
                                    <td class="text-center">
                                        <a class="btn btn-xs btn-success" data-toggle="tooltip" title="View Detail" href="{{route('admin.rolekkp.view.view', ['id' => $item->id])}}">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                        <a class="btn btn-xs btn-warning" data-toggle="tooltip" title="Edit" href="{{route('admin.rolekkp.edit.view', ['id' => $item->id])}}">
                                        ...
                                        {{route('rolekkp.delete.action', ['id' => $item->id])}}">

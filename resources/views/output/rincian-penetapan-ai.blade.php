                  ...
                  $(document).ready(function() {
                      $('select[name="strategi_bisnis"]').on('change', function() {
                          ...
                          if(strategi_bisnisID) {
                              $.ajax({
                                  url: "{{url('/output/rincian-penetapan-ai/ajax/')}}/"+strategi_bisnisID,
                                  type: "GET",
                                  dataType: "json",
            ...
            <script type="text/javascript">
               $(document).ready(function() {
                   $('select[name="distrik"]').on('change', function() {
                       ...
                       if(lokasiID) {
                           $.ajax({
                               url: "{{url('/output/rincian-penetapan-ai/ajax2/')}}/"+lokasiID,
                               type: "GET",
                               dataType: "json",
                               success:function(data) {
               ...
               <div class="col-md-4 col-sm-4 col-xs-12">
                 <!-- <select required="true" name="draft1" class="form-control col-md-7 col-xs-12">
                    <option>- Pilih Draft -</option>
                      @isset($idraft)
                        <option value="{{$idraft}}" selected> {{$draft->draft_versi}}</option>
                      @endisset
                    </select> -->

                    <input type="text" name="draft1" class="form-control col-md-12 col-xs-12" value="{{isset($idraft) ? $draft->draft_versi.' - '.$draft->name : ''}}" readonly="readonly" />
               </div>
               </div>
               <script type="text/javascript">
                  $(document).ready(function() {
                    $('select[name="lokasi"]').on('change', function() {
                      ...
                      if(id_lokasi && id_tahun) {
                          $.ajax({
                              url: "{{url('/output/rincian-penetapan-ai/ajax3/')}}/"+id_lokasi+"/"+id_tahun,
                              type: "GET",
                              dataType: "json",
                              success:function(data) {
              ...
              <script type="text/javascript">
                Highcharts.chart('target-terkontrak', {
                  ...
                  series: [ {
                      ...
                      //data: [4200, 8920, 13877, 13877, 14172, 19376, 19376, 20073, 26456, 32480, 17547, 17547]
                  },],
...
<!-- Table Rincian Penetapan Anggaran Penetapan Usaha -->

        <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel col-md-12">
                  <div class="x_title">
                    <h2 style="font-size: 18px;">RINCIAN PENETAPAN ANGGARAN INVESTASI PJB</h2>
                    <ul class="nav navbar-right panel_toolbox">
                  ...
                  <a href="{{ Request::fullUrl() }}&download=rincian-penetapan-ai&type=excel" target="_blank" id="get-excel" class="btn btn-success pull-right"><i class="fa fa-download"></i> Download Excel</a>

                  <a href="{{ Request::fullUrl() }}&download=rincian-penetapan-ai&type=pdf" target="_blank" id="get-pdf" class="btn btn-primary pull-right"><i class="fa fa-download"></i> Download PDF</a>

                  <div class="x_content">
                       <div class="col-md-12">
                        {{-- <div class="table-responsive"> --}}

                         <table class="table table-striped table-bordered" cellspacing="0" width="100%" style="height: 100px !important;font-size:11px;" id="table-ai" >
                              ...
                              if( $count != NULL ) {
                                ...
                                for ($i = 0 ; $i < $count; $i++) {
                                  ...
                                  <tr style="background-color: <?= $warna?>">
                                    ...
                                    <td style="text-align: center;">{{$targetBulan[$i]->value}}</td>
                                    <td style="text-align: center;">{{$targetTahun[$i]->value}}</td>
                                    <td style="text-align: center;">{{$levering[$i]->value}}</td>
                                    <td style="text-align: center;">{{$pengadaanPusat[$i]->value}}</td>
                                    <td style="text-align: center;">{{$disburseBulan[$i]->value}}</td>

...
// //  CHANGE REQUEST C-066842 start september 2020
if (!function_exists('catCode')) {
    function shortenCatCode($catcode)
    {
        switch ($catcode) {
            case 'Beban (MAT)':
                return 'MAT';
...
if (!function_exists('isCostLabel')) {
    function isCostLabel($label)
    {
        return in_array($label, [
            'Direct Est Cost/Revenue',
            'Category Value',
            'Total Year Estimate',
            'Jan',
            'Feb',
...
if (!function_exists('textEllipsis')) {
    function textEllipsis($text, $length = 16)
    {
        return strlen($text) > $length ? substr($text, 0, $length)."..." : $text;
    }
}

//  CHANGE REQUEST C-066842 end september 2020

//20200131 - Deploy function change request Loader Ellipse
if (!function_exists('loaderEllipsePgdlVal')) {
    function loaderEllipsePgdlVal($prk_type, $catcode, $label, $column, $rowData, $strlength = false, $strstart = 0)
    {
        if ($label == 'Category Code'){
            return shortenCatCode($catcode);
        } elseif ($label == 'Direct Est Cost/Revenue' || $label == 'Category Value') {
            $value = floatval($rowData['Beban (MAT)']) + floatval($rowData['Ijin Proses (LAB)']);
        } elseif ($label == 'Total Year Estimate' && ($prk_type == 'parent' || $prk_type == 'inti')) {
            $value = 0;
        } elseif ($column && isset($rowData[$column])) {
            $value = $rowData[$column];
        } else {
            return '';
        }

        if ($column == 'Nomor Project / PRK' && $strlength) {
            return substr($value, $strstart, $strlength);
        } elseif ( $column == 'Nomor Project / PRK' || $column == 'Parent Project' || $column == 'Tahun Disburse' || $column == 'Years') {
            return $value;
        } elseif (isCostLabel($label)) {
            $value = intval((float)$value * 1000);
...
if (!function_exists('loaderEllipsePgdlValPlain')) {
    function loaderEllipsePgdlValPlain($prk_type, $catcode, $label, $column, $rowData, $strlength = false, $strstart = 0)
    {
        if ($label == 'Category Code'){
            return shortenCatCode($catcode);
        } elseif ($label == 'Direct Est Cost/Revenue' || $label == 'Category Value') {
        ...
        if ( $column == 'Nomor Project / PRK' || $column == 'Parent Project' || $column == 'Tahun Disburse' || $column == 'Years') {
            if ($strlength) {
                return substr($value, $strstart, $strlength);
            } else {
                return $value;
            ...
            return intval((float)$value * 1000);
        } else {
            return $value;
        }
    }

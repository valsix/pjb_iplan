...
class KkpController extends Controller
{
    public function index()
    {
        ...
        if (($role_id=='5' && $grupdiv_id=='4')) 
        {
            ...
            if($input_tahun != null){
                    ...
                    $dmr_non_review = Dmr::where('tahun_anggaran', $input_tahun)
                        ->where('is_kkp', '1')
                ...
                {
                    $dmr = Dmr::where('tahun_anggaran', $input_tahun)
                        ->where('is_kkp', '1')
                        ->orderBy('id', 'desc')
                        ->get();
                        //dd($dmr);
        ...
        $revised_by= DB::table('approval_kkp')
        ->select('approval_kkp.grupdiv_id', 'grup_divpembinaunit.name as grupdiv_name', 'approval_kkp.peran', 'approval_kkp.urutan', 'approval_kkp.status', 'approval_kkp.kkp_id')
        ->join('grup_divpembinaunit', 'grup_divpembinaunit.id', '=', 'approval_kkp.grupdiv_id')
        ->where('approval_kkp.status', 2)
        ->whereRaw("approval_kkp.urutan = (select max(a.urutan) from approval_kkp as a where a.kkp_id = approval_kkp.kkp_id and a.status = '2')")
        ->orderBy('urutan', 'asc')
        ->get();
        ...
        if ($dmr) 
        {
            foreach ($dmr as $key => $value) 
            {
                ...
                $grupdivrevb= $grupdivrevbname= "";
                foreach ($revised_by as $valrevb) 
                {
                    if ($value->id == $valrevb->kkp_id) 
                    {
                        $grupdivrevbname= $valrevb->grupdiv_name;

                        if(empty($grupdivrevb))
                            $grupdivrevb= $grupdivrevbname;
                        else
                        {
                            $grupdivrevb= $grupdivrevb.", ".$grupdivrevbname;
                        }
                    }
                }
                $datrevb[$key]['kkp_id']= $value->id;
                $datrevb[$key]['grupdivname']= $grupdivrevb;
            }
        }
        
        
        //dd($dmr);
        return view('kkp.daftar', compact('Sb', 'distrik', 'lokasi', 'input_sb', 'input_distrik', 'input_lokasi', 'input_tahun', 'dmr', 'role_spv_unit_dmr_tor', 'role_id', 'grupdiv_id', 'datrs', 'review_skr', 'datrb', 'required', 'datrevb', 'bagian', 'input_bagian'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            $bidang_divisi_id = $user->bidang_divisi->id;

            $distrik = Distrik::find($distrik_id);
            $bidang_divisi = BidangDivisi::find($bidang_divisi_id);

            $item ['lokasi'] = Lokasi::where("distrik_id", $distrik_id)->get();
            // $item ['dmr'] = Dmr::all();
            // $item ['dmrattachment'] = DmrAttachment::all();
            return view('kkp.tambah', $item,compact('distrik', 'bidang_divisi'));
        }
        ...
        elseif ($request->isMethod('post')) {
            ...
            $distrik_id = Input::get('distrik_id');
            $bidang_divisi_id = null;
            if ($distrik_id==28 || $distrik_id==21) 
            {
                $bidang_divisi_id = Input::get('bidang_divisi_id');
            }
    ...
    public function update(Request $request,$id)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            return view('kkp.edit', compact('dmr', 'dmr_review', 'lokasi', 'dmrattachment', 'bidang_divisi'));
        }
        ...
        elseif ($request->isMethod('post')) {
            ...
            $bidang_divisi=null;
            if ($item->lokasi->distrik_id==21 || $item->lokasi->distrik_id==28) 
            {
                $bidang_divisi= BidangDivisi::find($item->bidang_divisi_id);
            }

            $request->session()->flash('success','Data berhasil diupdate');
            return redirect('kkp/daftar?tahun_anggaran='.$item->tahun_anggaran.'&strategi_bisnis='.$item->lokasi->distrik->strategi_bisnis_id.'&distrik='.$item->lokasi->distrik_id.'&lokasi='.$item->lokasi_id.'&bidang_divisi='.$bidang_divisi);
        }
    }
    ...
    public function detail(Request $request,$id)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            $array_status_dmr = [DMR_STATUS_REVISED, DMR_STATUS_REJECTED, 8];
            ...
            if ($kondisi=='1' OR $kondisi=='') 
            {
                ...
                ->whereNotIn('approval_kkp.status', ['2'])
                ->orderBy('urutan', 'asc')
                ->get();
            ...
            // return view('kkp.detail', compact('role_id', 'dmr', 'dmrattachment', 'dmr_reviews', 'input_dmr_review', 'tor_attachments'));
        }
        ...
        elseif ($request->isMethod('post')) {
            ...
            // return view('kkp.detail', compact('role_id', 'dmr', 'dmrattachment', 'dmr_reviews', 'input_dmr_review', 'dmr_review', 'tor_attachments'));
        }

        return view('kkp.detail', compact('role_id', 'dmr', 'dmrattachment', 'dmr_reviews', 'input_dmr_review', 'dmr_review', 'tor_attachments', 'dat_approval', 'dat_approval2', 'grupdiv', 'disabled', 'fungsi', 'bidang_divisi'));
    }
    ...
    public function setappr(Request $request,$id)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            $disabled= 'disabled';
            if ($dmr->status_appr_id=='' || ($dmr->status_appr_id=='4' && ($dmr->kondisi_aicluster_id=='2' || $dmr->kondisi_aicluster_id=='3')) ) 
            {
                $disabled= '';
            }
        ...
        elseif ($request->isMethod('post')) {
            ...
            // if ($inputke=='') 
            // {
            //     if (count($dat_appr_peran) > 0) {
            //         foreach ($dat_appr_peran as $nilai) {
            //             if ($nilai=='approval') {
            //                 $jumlahKemunculan = array_count_values($dat_appr_peran)[$nilai];
            //                 if ($jumlahKemunculan >= 2) {
            //                     $request->session()->flash('fail', 'Pada kondisi Nilai AI dibawah 50M, Peran Approval tidak boleh lebih dari 1');

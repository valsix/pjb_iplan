...
class RekapLRV2
{
  public static function index($file_imports_pgdl, $input_tahun, $input_lokasi, $int_input_bulan, $input_bulan, $strategi_bisnis, $distrik, $settings, $result_data, $result_data_ketetapan)
  {
    ...
    // dd($result_data, $result_data_ketetapan);
        ...
        foreach ($settings as $key_setting => $column_setting) {
          ...
          elseif ($column_setting->pgdl_report_dashboard_source_id == 5) {
            ...
            if ($kolom_bulan) {
              ...
              elseif ($key == 22) // rutin
                $temp[$column_setting->judul_kolom] = RekapLRQuery::get_rkap_sd_bulan_non_luar_jawa($file_imports_pgdl, $input_tahun, $increment_month, 'I-Pendapatan', 'M', 20, $input_lokasi->id);
              elseif ($key == 23) // reimburse
                $temp[$column_setting->judul_kolom] = RekapLRQuery::get_rkap_sd_bulan_non_luar_jawa($file_imports_pgdl, $input_tahun, $increment_month, 'I-Pendapatan', 'M', 21, $input_lokasi->id);
              ...
              elseif ($key == 45)
                ...
                $temp[$column_setting->judul_kolom] = RekapLRQuery::get_rkap_sd_bulan_non_luar_jawa($file_imports_pgdl, $input_tahun, $increment_month, 'I-PEG', 'M', 75, $input_lokasi->id);
              elseif ($key == 46)
              ...
              elseif ($key == 49) // tambahan untuk Pembelian tenaga listrik
                $temp[$column_setting->judul_kolom] = RekapLRQuery::get_rkap_sd_bulan_non_luar_jawa($file_imports_pgdl, $input_tahun, $increment_month, 'I-Pendapatan', 'M', 22, $input_lokasi->id);
              elseif ($key == 55)
    ...
    foreach ($settings as $key => $column_setting) {
      if ($column_setting->judul_kolom != 'Keterangan' && $column_setting->judul_kolom != 'RKAP n' && $column_setting->judul_kolom != 'RKAP n Update') {
        ...
        //$lr_result[28] = 3.1 Bahan bakar
        //$lr_result[38] = 3.2 Pemeliharaan
        //					
        // 
        // PENJUMLAHAN DI DASHBOARD REKAP LR
        for ($i = 16; $i <= 20; $i++) {
        ...
        for ($i = 29; $i <= 37; $i++) {
          //$lr_result[28] adalah untuk penjumlahan dari dashboard 3.1 bahan bakar 
        ...
        // penjumlahan pemeliharaan rekap lr
        ...
        // $lr_result[26] BEBAN USAHA penjumalah dari point-point dibawahnya
